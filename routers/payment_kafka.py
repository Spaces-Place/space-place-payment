...
@payment_kafka_router.post(
    "/kakao", response_model=Dict, status_code=status.HTTP_200_OK, summary="결제 준비"
)
async def payment_ready(
    ...
    try:
        ...
        ready_approval_data = {"payment_id": payment_id, "order_number": order_number}
...
async def payment_approve(
    ...
    return PaymentApproveResponse(
        message="예약 및 결제가 완료되었습니다.", order_number=order_number
    )
...
async def payment_fail(
    ...
    # 결제 실패
    await payment_service.ready_fail(
        json.dumps({"order_number": fail_data.order_number})
    )
...
@payment_kafka_router.get(
    "", response_model=Dict, status_code=status.HTTP_200_OK, summary="결제 내역 확인"
)
async def get_reservations(
    ...
    statement = (
        select(Payment)
        .where(Payment.user_id == token_info["user_id"])
        .offset(skip)
        .limit(limit)
    )
    result = await session.execute(statement)
